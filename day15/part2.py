...
def wrap_9(x: int) -> int:
    return (x - 1) % 9 + 1


def expand_rows(text: str) -> Iterable[str]:
    for i in range(5):
        for char in text:
            yield str(wrap_9(int(char) + i))


def expand_columns(numbers: Tuple[int, ...]) -> Iterable[int]:
    for i in range(5):
        for number in numbers:
            yield wrap_9(number + i)


def solve(puzzle_input: Union[str, Path]) -> int:
    numbers, line_length = parse_array(
        text=read(puzzle_input), target=int, line_splitter=expand_rows
    )
    line_length = line_length * 5
    numbers = tuple(expand_columns(numbers=numbers))
    start = 0
    goal = len(numbers) - 1
...
if __name__ == "__main__":
    ...
    if test_answer == TEST_RESULT:
        ...
        print(answer, f"found in {end-start:.2f} seconds.")
